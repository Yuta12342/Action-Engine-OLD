...
class OptionsLua {
	...
	#if hscript
	public static var hscript:OptionsHScript = null;
	#end
	...
	public function new(script:String) {
		...
		//set('bpm', options.OptionsState.SONG.bpm);
		//set('scrollSpeed', options.OptionsState.SONG.speed);
		set('crochet', Conductor.crochet);
		set('stepCrochet', Conductor.stepCrochet);
		set('songLength', FlxG.sound.music.length);
		//set('songName', options.OptionsState.SONG.song);
		//set('songPath', Paths.formatToSongPath(options.OptionsState.SONG.song));
		set('startedCountdown', false);
		//set('mania', options.OptionsState.mania);
		//set('curStage', options.OptionsState.SONG.stage);

		//set('isStoryMode', options.OptionsState.isStoryMode);
		//set('difficulty', options.OptionsState.storyDifficulty);

		//var difficultyName:String = CoolUtil.difficulties[options.OptionsState.storyDifficulty];
		//set('difficultyName', difficultyName);
		//set('difficultyPath', Paths.formatToSongPath(difficultyName));
		//set('weekRaw', options.OptionsState.storyWeek);
		//set('week', WeekData.weeksList[options.OptionsState.storyWeek]);
		//set('seenCutscene', options.OptionsState.seenCutscene);
		...
		// options.OptionsState cringe ass nae nae bullcrap
		set('curBeat', 0);
		set('curStep', 0);
		set('curDecBeat', 0);
		set('curDecStep', 0);
		...
		set('ratingName', '');
		set('ratingFC', '');
		//set('version', options.OptionsState.psychEngineVersion.trim());
		...
		// Gameplay settings
		//set('healthGainMult', options.OptionsState.instance.healthGain);
		//set('healthLossMult', options.OptionsState.instance.healthLoss);
		//set('playbackRate', options.OptionsState.instance.playbackRate);
		//set('instakillOnMiss', options.OptionsState.instance.instakillOnMiss);
		//set('botPlay', options.OptionsState.instance.cpuControlled);
		//set('practice', options.OptionsState.instance.practiceMode);

		/*for (i in 0...options.OptionsState.mania) {
		...
		//set('defaultBoyfriendX', options.OptionsState.instance.BF_X);
		//set('defaultBoyfriendY', options.OptionsState.instance.BF_Y);
		//set('defaultOpponentX', options.OptionsState.instance.DAD_X);
		//set('defaultOpponentY', options.OptionsState.instance.DAD_Y);
		//set('defaultGirlfriendX', options.OptionsState.instance.GF_X);
		//set('defaultGirlfriendY', options.OptionsState.instance.GF_Y);

		// Character shit
		//set('boyfriendName', options.OptionsState.SONG.player1);
		//set('dadName', options.OptionsState.SONG.player2);
		//set('gfName', options.OptionsState.SONG.gfVersion);
		...
		Lua_helper.add_callback(lua, "openCustomSubstate", function(name:String, pauseGame:Bool = false) {
			if(pauseGame)
			{
				options.OptionsState.instance.persistentUpdate = false;
				options.OptionsState.instance.persistentDraw = true;
				/*options.OptionsState.instance.paused = true;
				if(FlxG.sound.music != null) {
					FlxG.sound.music.pause();
					options.OptionsState.instance.vocals.pause();
				}*/
			}
			options.OptionsState.instance.openSubState(new OptionsCustomSubstate(name));
		});

		Lua_helper.add_callback(lua, "closeCustomSubstate", function() {
			if(OptionsCustomSubstate.instance != null)
			{
				options.OptionsState.instance.closeSubState();
				OptionsCustomSubstate.instance = null;
				return true;
			}
			return false;
		});
		...
		Lua_helper.add_callback(lua, "setSpriteShader", function(obj:String, shader:String) {
			...
			#if (!flash && MODS_ALLOWED && sys)
			if(!options.OptionsState.instance.runtimeShaders.exists(shader) && !initLuaShader(shader))
			{
			...
			if(leObj != null) {
				var arr:Array<String> = options.OptionsState.instance.runtimeShaders.get(shader);
		...
		Lua_helper.add_callback(lua, "getRunningScripts", function(){
			...
			for (idx in 0...options.OptionsState.instance.luaArray.length)
				runningScripts.push(options.OptionsState.instance.luaArray[idx].scriptName);


			return runningScripts;
		});
		/*Lua_helper.add_callback(lua, "addSongToOptions", function(songName:String, weekNum:Int, songCharacter:String, colors:Array<Int>){
			var colors:Array<Int> = colors;
			if(colors == null || colors.length < 3)
			{
				colors = [146, 113, 253];
			}
			options.OptionsState.songs.push(new options.OptionsState.SongMetadata(songName, weekNum, songCharacter, FlxColor.fromRGB(colors[0], colors[1], colors[2])));
		});*/
		...
		Lua_helper.add_callback(lua, "callOnLuas", function(?funcName:String, ?args:Array<Dynamic>, ignoreStops=false, ignoreSelf=true, ?exclusions:Array<String>){
			...
			options.OptionsState.instance.callOnLuas(funcName, args, ignoreStops, exclusions);
		});
		...
		Lua_helper.add_callback(lua, "callScript", function(?luaFile:String, ?funcName:String, ?args:Array<Dynamic>){
			...
			if(doPush)
			{
				for (luaInstance in options.OptionsState.instance.luaArray)
				{
		...
		Lua_helper.add_callback(lua, "addLuaScript", function(luaFile:String, ?ignoreAlreadyRunning:Bool = false) { //would be dope asf.
			...
			if(doPush)
			{
				if(!ignoreAlreadyRunning)
				{
					...
					options.OptionsState.instance.luaArray.push(new OptionsLua(cervix));
					return;
				}
				...
				if(!ignoreAlreadyRunning)
				{
					for (luaInstance in options.OptionsState.instance.luaArray)
					{
						if(luaInstance.scriptName == cervix)
						{
							...
							options.OptionsState.instance.luaArray.remove(luaInstance);
							return;
						}
					}
				}
		...
		Lua_helper.add_callback(lua, "doTweenX", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			...
			if(penisExam != null) {
				options.OptionsState.instance.modchartTweens.set(tag, FlxTween.tween(penisExam, {x: value}, duration, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						options.OptionsState.instance.callOnLuas('onTweenCompleted', [tag]);
						options.OptionsState.instance.modchartTweens.remove(tag);
					}
				}));
			} else {
		...
		Lua_helper.add_callback(lua, "doTweenY", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
				options.OptionsState.instance.modchartTweens.set(tag, FlxTween.tween(penisExam, {y: value}, duration, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						options.OptionsState.instance.callOnLuas('onTweenCompleted', [tag]);
						options.OptionsState.instance.modchartTweens.remove(tag);
					}
		...
		Lua_helper.add_callback(lua, "doTweenAngle", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
				options.OptionsState.instance.modchartTweens.set(tag, FlxTween.tween(penisExam, {angle: value}, duration, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						options.OptionsState.instance.callOnLuas('onTweenCompleted', [tag]);
						options.OptionsState.instance.modchartTweens.remove(tag);
					}
		...
		Lua_helper.add_callback(lua, "doTweenAlpha", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
				options.OptionsState.instance.modchartTweens.set(tag, FlxTween.tween(penisExam, {alpha: value}, duration, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						options.OptionsState.instance.callOnLuas('onTweenCompleted', [tag]);
						options.OptionsState.instance.modchartTweens.remove(tag);
					}
		...
		Lua_helper.add_callback(lua, "doTweenZoom", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
				options.OptionsState.instance.modchartTweens.set(tag, FlxTween.tween(penisExam, {zoom: value}, duration, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						options.OptionsState.instance.callOnLuas('onTweenCompleted', [tag]);
						options.OptionsState.instance.modchartTweens.remove(tag);
					}
		...
		Lua_helper.add_callback(lua, "doTweenColor", function(tag:String, vars:String, targetColor:String, duration:Float, ease:String) {
			...
			if(penisExam != null) {
				...
				options.OptionsState.instance.modchartTweens.set(tag, FlxTween.color(penisExam, duration, curColor, color, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						options.OptionsState.instance.modchartTweens.remove(tag);
						options.OptionsState.instance.callOnLuas('onTweenCompleted', [tag]);
					}
		...
		Lua_helper.add_callback(lua, "runTimer", function(tag:String, time:Float = 1, loops:Int = 1) {
			cancelTimer(tag);
			options.OptionsState.instance.modchartTimers.set(tag, new FlxTimer().start(time, function(tmr:FlxTimer) {
				if(tmr.finished) {
					options.OptionsState.instance.modchartTimers.remove(tag);
				}
				options.OptionsState.instance.callOnLuas('onTimerCompleted', [tag, tmr.loops, tmr.loopsLeft]);
		...
		Lua_helper.add_callback(lua, "makeLuaSprite", function(tag:String, image:String, x:Float, y:Float) {
			tag = tag.replace('.', '');
			resetSpriteTag(tag);
			var leSprite:OptionsModchartSprite = new OptionsModchartSprite(x, y);
			if(image != null && image.length > 0)
			{
				leSprite.loadGraphic(Paths.image(image));
			}
			leSprite.antialiasing = ClientPrefs.globalAntialiasing;
			options.OptionsState.instance.modchartSprites.set(tag, leSprite);
		...
		Lua_helper.add_callback(lua, "makeGraphic", function(obj:String, width:Int, height:Int, color:String) {
			...
			var spr:FlxSprite = options.OptionsState.instance.getLuaObject(obj,false);
			if(spr!=null) {
				options.OptionsState.instance.getLuaObject(obj,false).makeGraphic(width, height, colorNum);
				return;
			}
		...
		Lua_helper.add_callback(lua, "addAnimationByPrefix", function(obj:String, name:String, prefix:String, framerate:Int = 24, loop:Bool = true) {
			if(options.OptionsState.instance.getLuaObject(obj,false)!=null) {
				var cock:FlxSprite = options.OptionsState.instance.getLuaObject(obj,false);
		...
		{
			if(options.OptionsState.instance.getLuaObject(obj, false) != null) {
				var luaObj:FlxSprite = options.OptionsState.instance.getLuaObject(obj,false);
				if(luaObj.animation.getByName(name) != null)
				{
					luaObj.animation.play(name, forced, reverse, startFrame);
					if(Std.isOfType(luaObj, OptionsModchartSprite))
					{
						//convert luaObj to ModchartSprite
						var obj:Dynamic = luaObj;
						var luaObj:OptionsModchartSprite = obj;
		...
		Lua_helper.add_callback(lua, "addOffset", function(obj:String, anim:String, x:Float, y:Float) {
			if(options.OptionsState.instance.modchartSprites.exists(obj)) {
				options.OptionsState.instance.modchartSprites.get(obj).animOffsets.set(anim, [x, y]);
				return true;
			}
		...
		Lua_helper.add_callback(lua, "setScrollFactor", function(obj:String, scrollX:Float, scrollY:Float) {
			if(options.OptionsState.instance.getLuaObject(obj,false)!=null) {
				options.OptionsState.instance.getLuaObject(obj,false).scrollFactor.set(scrollX, scrollY);
				return;
			}
		...
		Lua_helper.add_callback(lua, "addLuaSprite", function(tag:String, front:Bool = false) {
			if(options.OptionsState.instance.modchartSprites.exists(tag)) {
				var shit:OptionsModchartSprite = options.OptionsState.instance.modchartSprites.get(tag);
				if(!shit.wasAdded) {
					if(front)
					{
						options.OptionsState.instance.add(shit);
					}
					else
						{
							var position:Int = options.OptionsState.instance.members.indexOf(options.OptionsState.instance.bg);
							options.OptionsState.instance.insert(position + 1, shit);
						}
					shit.wasAdded = true;
		...
		Lua_helper.add_callback(lua, "setGraphicSize", function(obj:String, x:Int, y:Int = 0, updateHitbox:Bool = true) {
			if(options.OptionsState.instance.getLuaObject(obj)!=null) {
				var shit:FlxSprite = options.OptionsState.instance.getLuaObject(obj);
		...
		Lua_helper.add_callback(lua, "removeLuaSprite", function(tag:String, destroy:Bool = true) {
			...
			var pee:OptionsModchartSprite = options.OptionsState.instance.modchartSprites.get(tag);
			if(destroy) {
				pee.kill();
			}
			...
			if(destroy) {
				pee.destroy();
				options.OptionsState.instance.modchartSprites.remove(tag);
			}
		});

		Lua_helper.add_callback(lua, "luaSpriteExists", function(tag:String) {
			return options.OptionsState.instance.modchartSprites.exists(tag);
		});
		Lua_helper.add_callback(lua, "luaTextExists", function(tag:String) {
			return options.OptionsState.instance.modchartTexts.exists(tag);
		});
		Lua_helper.add_callback(lua, "luaSoundExists", function(tag:String) {
			return options.OptionsState.instance.modchartSounds.exists(tag);
		});


		Lua_helper.add_callback(lua, "setObjectCamera", function(obj:String, camera:String = '') {
			var real = options.OptionsState.instance.getLuaObject(obj);
			if(real!=null){
		...
		Lua_helper.add_callback(lua, "screenCenter", function(obj:String, pos:String = 'xy') {
			var spr:FlxSprite = options.OptionsState.instance.getLuaObject(obj);
		...
		Lua_helper.add_callback(lua, "objectsOverlap", function(obj1:String, obj2:String) {
			...
			{
				var real = options.OptionsState.instance.getLuaObject(namesArray[i]);
				if(real!=null) {
		...
		Lua_helper.add_callback(lua, "playSound", function(sound:String, volume:Float = 1, ?tag:String = null) {
			if(tag != null && tag.length > 0) {
				tag = tag.replace('.', '');
				if(options.OptionsState.instance.modchartSounds.exists(tag)) {
					options.OptionsState.instance.modchartSounds.get(tag).stop();
				}
				options.OptionsState.instance.modchartSounds.set(tag, FlxG.sound.play(Paths.sound(sound), volume, false, function() {
					options.OptionsState.instance.modchartSounds.remove(tag);
					options.OptionsState.instance.callOnLuas('onSoundFinished', [tag]);
				}));
				return;
			}
		...
		Lua_helper.add_callback(lua, "stopSound", function(tag:String) {
			if(tag != null && tag.length > 1 && options.OptionsState.instance.modchartSounds.exists(tag)) {
				options.OptionsState.instance.modchartSounds.get(tag).stop();
				options.OptionsState.instance.modchartSounds.remove(tag);
			}
		});
		Lua_helper.add_callback(lua, "pauseSound", function(tag:String) {
			if(tag != null && tag.length > 1 && options.OptionsState.instance.modchartSounds.exists(tag)) {
				options.OptionsState.instance.modchartSounds.get(tag).pause();
			}
		});
		Lua_helper.add_callback(lua, "resumeSound", function(tag:String) {
			if(tag != null && tag.length > 1 && options.OptionsState.instance.modchartSounds.exists(tag)) {
				options.OptionsState.instance.modchartSounds.get(tag).play();
			}
		});
		Lua_helper.add_callback(lua, "soundFadeIn", function(tag:String, duration:Float, fromValue:Float = 0, toValue:Float = 1) {
			...
			} else if(options.OptionsState.instance.modchartSounds.exists(tag)) {
				options.OptionsState.instance.modchartSounds.get(tag).fadeIn(duration, fromValue, toValue);
			}
		...
		Lua_helper.add_callback(lua, "soundFadeOut", function(tag:String, duration:Float, toValue:Float = 0) {
			if(tag == null || tag.length < 1) {
				...
				options.OptionsState.instance.modchartSounds.get(tag).fadeOut(duration, toValue);
			}
		});
		Lua_helper.add_callback(lua, "soundFadeCancel", function(tag:String) {
			if(tag == null || tag.length < 1) {
				...
				var theSound:FlxSound = options.OptionsState.instance.modchartSounds.get(tag);
		...
		Lua_helper.add_callback(lua, "getSoundVolume", function(tag:String) {
			if(tag == null || tag.length < 1) {
				...
				return options.OptionsState.instance.modchartSounds.get(tag).volume;
			}
			return 0;
		});
		Lua_helper.add_callback(lua, "setSoundVolume", function(tag:String, value:Float) {
			if(tag == null || tag.length < 1) {
				...
				options.OptionsState.instance.modchartSounds.get(tag).volume = value;
			}
		});
		Lua_helper.add_callback(lua, "getSoundTime", function(tag:String) {
			if(tag != null && tag.length > 0 && options.OptionsState.instance.modchartSounds.exists(tag)) {
				return options.OptionsState.instance.modchartSounds.get(tag).time;
			}
			return 0;
		});
		...
		Lua_helper.add_callback(lua, "makeLuaText", function(tag:String, text:String, width:Int, x:Float, y:Float) {
			tag = tag.replace('.', '');
			resetTextTag(tag);
			var leText:OptionsModchartText = new OptionsModchartText(x, y, text, width);
			options.OptionsState.instance.modchartTexts.set(tag, leText);
		});
		...
		Lua_helper.add_callback(lua, "addLuaText", function(tag:String) {
			if(options.OptionsState.instance.modchartTexts.exists(tag)) {
				var shit:OptionsModchartText = options.OptionsState.instance.modchartTexts.get(tag);
				if(!shit.wasAdded) {
		...
		Lua_helper.add_callback(lua, "removeLuaText", function(tag:String, destroy:Bool = true) {
			...
			var pee:OptionsModchartText = options.OptionsState.instance.modchartTexts.get(tag);
			if(destroy) {
				pee.kill();
			}
			...
			if(destroy) {
				pee.destroy();
				options.OptionsState.instance.modchartTexts.remove(tag);
			}
		});

		Lua_helper.add_callback(lua, "initSaveData", function(name:String, ?folder:String = 'psychenginemods') {
			if(!options.OptionsState.instance.modchartSaves.exists(name))
			{
				var save:FlxSave = new FlxSave();
				save.bind(name, folder);
				options.OptionsState.instance.modchartSaves.set(name, save);
				return;
			}
		...
		Lua_helper.add_callback(lua, "flushSaveData", function(name:String) {
			if(options.OptionsState.instance.modchartSaves.exists(name))
			{
				options.OptionsState.instance.modchartSaves.get(name).flush();
				return;
			}
		...
		Lua_helper.add_callback(lua, "getDataFromSave", function(name:String, field:String, ?defaultValue:Dynamic = null) {
			if(options.OptionsState.instance.modchartSaves.exists(name))
			{
				var retVal:Dynamic = Reflect.field(options.OptionsState.instance.modchartSaves.get(name).data, field);
				return retVal;
			}
		...
		Lua_helper.add_callback(lua, "setDataFromSave", function(name:String, field:String, value:Dynamic) {
			if(options.OptionsState.instance.modchartSaves.exists(name))
			{
				Reflect.setField(options.OptionsState.instance.modchartSaves.get(name).data, field, value);
				return;
			}
		...
		Lua_helper.add_callback(lua, "objectPlayAnimation", function(obj:String, name:String, forced:Bool = false, ?startFrame:Int = 0) {
			...
			if(options.OptionsState.instance.getLuaObject(obj,false) != null) {
				options.OptionsState.instance.getLuaObject(obj,false).animation.play(name, forced, false, startFrame);
				return true;
			}
		...
		Lua_helper.add_callback(lua, "luaSpriteMakeGraphic", function(tag:String, width:Int, height:Int, color:String) {
			...
			if(options.OptionsState.instance.modchartSprites.exists(tag)) {
				...
				options.OptionsState.instance.modchartSprites.get(tag).makeGraphic(width, height, colorNum);
			}
		});
		Lua_helper.add_callback(lua, "luaSpriteAddAnimationByPrefix", function(tag:String, name:String, prefix:String, framerate:Int = 24, loop:Bool = true) {
			...
			if(options.OptionsState.instance.modchartSprites.exists(tag)) {
				var cock:OptionsModchartSprite = options.OptionsState.instance.modchartSprites.get(tag);
				cock.animation.addByPrefix(name, prefix, framerate, loop);
				if(cock.animation.curAnim == null) {
					cock.animation.play(name, true);
				}
		...
		Lua_helper.add_callback(lua, "luaSpriteAddAnimationByIndices", function(tag:String, name:String, prefix:String, indices:String, framerate:Int = 24) {
			...
			if(options.OptionsState.instance.modchartSprites.exists(tag)) {
				...
				var pussy:OptionsModchartSprite = options.OptionsState.instance.modchartSprites.get(tag);
		...
		Lua_helper.add_callback(lua, "luaSpritePlayAnimation", function(tag:String, name:String, forced:Bool = false) {
			...
			if(options.OptionsState.instance.modchartSprites.exists(tag)) {
				options.OptionsState.instance.modchartSprites.get(tag).animation.play(name, forced);
			}
		});
		Lua_helper.add_callback(lua, "setLuaSpriteCamera", function(tag:String, camera:String = '') {
			...
			if(options.OptionsState.instance.modchartSprites.exists(tag)) {
				options.OptionsState.instance.modchartSprites.get(tag).cameras = [cameraFromString(camera)];
				return true;
			}
		...
		Lua_helper.add_callback(lua, "setLuaSpriteScrollFactor", function(tag:String, scrollX:Float, scrollY:Float) {
			...
			if(options.OptionsState.instance.modchartSprites.exists(tag)) {
				options.OptionsState.instance.modchartSprites.get(tag).scrollFactor.set(scrollX, scrollY);
				return true;
			}
			return false;
		});
		...
		Lua_helper.add_callback(lua, "getPropertyLuaSprite", function(tag:String, variable:String) {
			...
			if(options.OptionsState.instance.modchartSprites.exists(tag)) {
				var killMe:Array<String> = variable.split('.');
				if(killMe.length > 1) {
					var coverMeInPiss:Dynamic = Reflect.getProperty(options.OptionsState.instance.modchartSprites.get(tag), killMe[0]);
					for (i in 1...killMe.length-1) {
						coverMeInPiss = Reflect.getProperty(coverMeInPiss, killMe[i]);
					}
					return Reflect.getProperty(coverMeInPiss, killMe[killMe.length-1]);
				}
				return Reflect.getProperty(options.OptionsState.instance.modchartSprites.get(tag), variable);
			}
			return null;
		});
		Lua_helper.add_callback(lua, "setPropertyLuaSprite", function(tag:String, variable:String, value:Dynamic) {
			...
			if(options.OptionsState.instance.modchartSprites.exists(tag)) {
				...
				Reflect.setProperty(options.OptionsState.instance.modchartSprites.get(tag), variable, value);
				return true;
			}
	...
	public function initHaxeModule()
	{
		if(hscript == null)
		{
			...
			hscript = new OptionsHScript(); //TO DO: Fix issue with 2 scripts not being able to use the same variable names
		}
	}
	#end
	...
	public static function setVarInArray(instance:Dynamic, variable:String, value:Dynamic):Any
	{
		...
		if(shit.length > 1)
		{
			...
			if(options.OptionsState.instance.variables.exists(shit[0]))
			{
				var retVal:Dynamic = options.OptionsState.instance.variables.get(shit[0]);
				if(retVal != null)
					blah = retVal;
			}
			else
		...
		if(options.OptionsState.instance.variables.exists(variable))
		{
			options.OptionsState.instance.variables.set(variable, value);
			return true;
		}
	...
	public static function getVarInArray(instance:Dynamic, variable:String):Any
	{
		...
		if(options.OptionsState.instance.variables.exists(variable))
		{
			var retVal:Dynamic = options.OptionsState.instance.variables.get(variable);
			if(retVal != null)
				return retVal;
		}
	...
	{
		return options.OptionsState.instance.modchartTexts.exists(name) ? options.OptionsState.instance.modchartTexts.get(name) : Reflect.getProperty(options.OptionsState.instance, name);
	}
	...
	function initLuaShader(name:String, ?glslVersion:Int = 120)
	{
		...
		#if (!flash && sys)
		if(options.OptionsState.instance.runtimeShaders.exists(name))
		{
		...
		for (folder in foldersToCheck)
		{
			if(FileSystem.exists(folder))
			{
				...
				if(found)
				{
					options.OptionsState.instance.runtimeShaders.set(name, [frag, vert]);
	...
	function resetTextTag(tag:String) {
		...
		pee.kill();
		if(pee.wasAdded) {
			options.OptionsState.instance.remove(pee, true);
		}
		pee.destroy();
		options.OptionsState.instance.modchartTexts.remove(tag);
	}
	...
	function cancelTween(tag:String) {
		if(options.OptionsState.instance.modchartTweens.exists(tag)) {
			options.OptionsState.instance.modchartTweens.get(tag).cancel();
			options.OptionsState.instance.modchartTweens.get(tag).destroy();
			options.OptionsState.instance.modchartTweens.remove(tag);
		}
	}
	...
	function cancelTimer(tag:String) {
		if(options.OptionsState.instance.modchartTimers.exists(tag)) {
			var theTimer:FlxTimer = options.OptionsState.instance.modchartTimers.get(tag);
			theTimer.cancel();
			theTimer.destroy();
			options.OptionsState.instance.modchartTimers.remove(tag);
		}
	...
	function cameraFromString(cam:String):FlxCamera {
		switch(cam.toLowerCase()) {
			case 'camhud' | 'hud': return options.OptionsState.instance.camHUD;
			case 'camother' | 'other': return options.OptionsState.instance.camOther;
		}
		return options.OptionsState.instance.camGame;
	}
	...
	public function luaTrace(text:String, ignoreCheck:Bool = false, deprecated:Bool = false, color:FlxColor = FlxColor.WHITE) {
		...
		if(ignoreCheck || getBool('luaDebugMode')) {
			...
			options.OptionsState.instance.addTextToDebug(text, color);
			trace(text);
		}
		#end
	}
	...
	static function addAnimByIndices(obj:String, name:String, prefix:String, indices:String, framerate:Int = 24, loop:Bool = false)
	{
		...
		if(options.OptionsState.instance.getLuaObject(obj, false)!=null) {
			var pussy:FlxSprite = options.OptionsState.instance.getLuaObject(obj, false);
	...
	{
		var coverMeInPiss:Dynamic = options.OptionsState.instance.getLuaObject(objectName, checkForTextsToo);
	...
	{
		return options.OptionsState.instance;
	}
}

class OptionsModchartSprite extends FlxSprite
{
...
class OptionsModchartText extends FlxText
{
	public var wasAdded:Bool = false;
	public function new(x:Float, y:Float, text:String, width:Float)
	{
		...
		cameras = [options.OptionsState.instance.camHUD];
		scrollFactor.set();
		borderSize = 2;
	}
}

class OptionsDebugLuaText extends FlxText
{
	private var disableTime:Float = 6;
	public var parentGroup:FlxTypedGroup<OptionsDebugLuaText>;
	public function new(text:String, parentGroup:FlxTypedGroup<OptionsDebugLuaText>, color:FlxColor) {
...
class OptionsCustomSubstate extends MusicBeatSubstate
{
	public static var name:String = 'unnamed';
	public static var instance:OptionsCustomSubstate;

	override function create()
	{
		instance = this;

		options.OptionsState.instance.callOnLuas('onCustomSubstateCreate', [name]);
		super.create();
		options.OptionsState.instance.callOnLuas('onCustomSubstateCreatePost', [name]);
	}
	
	public function new(name:String)
	{
		OptionsCustomSubstate.name = name;
		super();
	...
	override function update(elapsed:Float)
	{
		options.OptionsState.instance.callOnLuas('onCustomSubstateUpdate', [name, elapsed]);
		super.update(elapsed);
		options.OptionsState.instance.callOnLuas('onCustomSubstateUpdatePost', [name, elapsed]);
	}

	override function destroy()
	{
		options.OptionsState.instance.callOnLuas('onCustomSubstateDestroy', [name]);
		super.destroy();
	}
}

#if hscript
class OptionsHScript
{
	...
	public function new()
	{
		...
		interp.variables.set('options.OptionsState', options.OptionsState);
		interp.variables.set('game', options.OptionsState.instance);
		interp.variables.set('Paths', Paths);
		interp.variables.set('Conductor', Conductor);
		interp.variables.set('ClientPrefs', ClientPrefs);
		interp.variables.set('Character', Character);
		interp.variables.set('Alphabet', Alphabet);
		interp.variables.set('CustomSubstate', OptionsCustomSubstate);
		#if (!flash && sys)
		...
		{
			options.OptionsState.instance.variables.set(name, value);
		});
		interp.variables.set('getVar', function(name:String)
		{
			var result:Dynamic = null;
			if(options.OptionsState.instance.variables.exists(name)) result = options.OptionsState.instance.variables.get(name);
			return result;
		});
		interp.variables.set('removeVar', function(name:String)
		{
			if(options.OptionsState.instance.variables.exists(name))
			{
				options.OptionsState.instance.variables.remove(name);
				return true;
			}
			return false;
		});
	...
	{
		@:privateAccess
		OptionsHScript.parser.line = 1;
		OptionsHScript.parser.allowTypes = true;
		return interp.execute(OptionsHScript.parser.parseString(codeToRun));
	}
}
#end
