...
class MenuLua {
	...
	#if hscript
	public static var hscript:MenuHScript = null;
	#end
	...
	public function new(script:String) {
		...
		//set('bpm', MainMenuState.SONG.bpm);
		//set('scrollSpeed', MainMenuState.SONG.speed);
		set('crochet', Conductor.crochet);
		set('stepCrochet', Conductor.stepCrochet);
		set('songLength', FlxG.sound.music.length);
		//set('songName', MainMenuState.SONG.song);
		//set('songPath', Paths.formatToSongPath(MainMenuState.SONG.song));
		set('startedCountdown', false);
		//set('mania', MainMenuState.mania);
		//set('curStage', MainMenuState.SONG.stage);

		//set('isStoryMode', MainMenuState.isStoryMode);
		//set('difficulty', MainMenuState.storyDifficulty);

		//var difficultyName:String = CoolUtil.difficulties[MainMenuState.storyDifficulty];
		//set('difficultyName', difficultyName);
		//set('difficultyPath', Paths.formatToSongPath(difficultyName));
		//set('weekRaw', MainMenuState.storyWeek);
		//set('week', WeekData.weeksList[MainMenuState.storyWeek]);
		//set('seenCutscene', MainMenuState.seenCutscene);
		...
		// MainMenuState cringe ass nae nae bullcrap
		set('curBeat', 0);
		set('curStep', 0);
		set('curDecBeat', 0);
		set('curDecStep', 0);
		...
		// Gameplay settings
		//set('healthGainMult', MainMenuState.instance.healthGain);
		//set('healthLossMult', MainMenuState.instance.healthLoss);
		//set('playbackRate', MainMenuState.instance.playbackRate);
		//set('instakillOnMiss', MainMenuState.instance.instakillOnMiss);
		//set('botPlay', MainMenuState.instance.cpuControlled);
		//set('practice', MainMenuState.instance.practiceMode);

		/*for (i in 0...MainMenuState.mania) {
		...
		//set('defaultBoyfriendX', MainMenuState.instance.BF_X);
		//set('defaultBoyfriendY', MainMenuState.instance.BF_Y);
		//set('defaultOpponentX', MainMenuState.instance.DAD_X);
		//set('defaultOpponentY', MainMenuState.instance.DAD_Y);
		//set('defaultGirlfriendX', MainMenuState.instance.GF_X);
		//set('defaultGirlfriendY', MainMenuState.instance.GF_Y);

		// Character shit
		//set('boyfriendName', MainMenuState.SONG.player1);
		//set('dadName', MainMenuState.SONG.player2);
		//set('gfName', MainMenuState.SONG.gfVersion);
		...
		Lua_helper.add_callback(lua, "openCustomSubstate", function(name:String, pauseGame:Bool = false) {
			if(pauseGame)
			{
				...
				if(FlxG.sound.music != null) {
					...
					MainMenuState.instance.vocals.pause();
				}*/
			}
			MainMenuState.instance.openSubState(new MenuCustomSubstate(name));
		});

		Lua_helper.add_callback(lua, "closeCustomSubstate", function() {
			if(MenuCustomSubstate.instance != null)
			{
				MainMenuState.instance.closeSubState();
				MenuCustomSubstate.instance = null;
				return true;
			}
			return false;
		});
		...
		Lua_helper.add_callback(lua, "setSpriteShader", function(obj:String, shader:String) {
			...
			#if (!flash && MODS_ALLOWED && sys)
			if(!MainMenuState.instance.runtimeShaders.exists(shader) && !initLuaShader(shader))
			{
			...
			if(leObj != null) {
				var arr:Array<String> = MainMenuState.instance.runtimeShaders.get(shader);
		...
		Lua_helper.add_callback(lua, "getRunningScripts", function(){
			...
			for (idx in 0...MainMenuState.instance.luaArray.length)
				runningScripts.push(MainMenuState.instance.luaArray[idx].scriptName);
		...
		Lua_helper.add_callback(lua, "callOnLuas", function(?funcName:String, ?args:Array<Dynamic>, ignoreStops=false, ignoreSelf=true, ?exclusions:Array<String>){
			...
			MainMenuState.instance.callOnLuas(funcName, args, ignoreStops, exclusions);
		});
		...
		Lua_helper.add_callback(lua, "callScript", function(?luaFile:String, ?funcName:String, ?args:Array<Dynamic>){
			...
			if(doPush)
			{
				for (luaInstance in MainMenuState.instance.luaArray)
				{
		...
		Lua_helper.add_callback(lua, "addLuaScript", function(luaFile:String, ?ignoreAlreadyRunning:Bool = false) { //would be dope asf.
			...
			if(doPush)
			{
				if(!ignoreAlreadyRunning)
				{
					...
					MainMenuState.instance.luaArray.push(new MenuLua(cervix));
					return;
				}
				...
				if(!ignoreAlreadyRunning)
				{
					for (luaInstance in MainMenuState.instance.luaArray)
					{
						if(luaInstance.scriptName == cervix)
						{
							...
							MainMenuState.instance.luaArray.remove(luaInstance);
							return;
						}
					}
				}
		...
		Lua_helper.add_callback(lua, "doTweenX", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			...
			if(penisExam != null) {
				MainMenuState.instance.modchartTweens.set(tag, FlxTween.tween(penisExam, {x: value}, duration, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						MainMenuState.instance.callOnLuas('onTweenCompleted', [tag]);
		...
		Lua_helper.add_callback(lua, "doTweenY", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
				MainMenuState.instance.modchartTweens.set(tag, FlxTween.tween(penisExam, {y: value}, duration, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						MainMenuState.instance.callOnLuas('onTweenCompleted', [tag]);
						MainMenuState.instance.modchartTweens.remove(tag);
					}
		...
		Lua_helper.add_callback(lua, "doTweenAngle", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
				MainMenuState.instance.modchartTweens.set(tag, FlxTween.tween(penisExam, {angle: value}, duration, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						MainMenuState.instance.callOnLuas('onTweenCompleted', [tag]);
						MainMenuState.instance.modchartTweens.remove(tag);
					}
		...
		Lua_helper.add_callback(lua, "doTweenAlpha", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
				MainMenuState.instance.modchartTweens.set(tag, FlxTween.tween(penisExam, {alpha: value}, duration, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						MainMenuState.instance.callOnLuas('onTweenCompleted', [tag]);
						MainMenuState.instance.modchartTweens.remove(tag);
					}
		...
		Lua_helper.add_callback(lua, "doTweenZoom", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
				MainMenuState.instance.modchartTweens.set(tag, FlxTween.tween(penisExam, {zoom: value}, duration, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						MainMenuState.instance.callOnLuas('onTweenCompleted', [tag]);
						MainMenuState.instance.modchartTweens.remove(tag);
					}
		...
		Lua_helper.add_callback(lua, "doTweenColor", function(tag:String, vars:String, targetColor:String, duration:Float, ease:String) {
			...
			if(penisExam != null) {
				...
				MainMenuState.instance.modchartTweens.set(tag, FlxTween.color(penisExam, duration, curColor, color, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						MainMenuState.instance.modchartTweens.remove(tag);
						MainMenuState.instance.callOnLuas('onTweenCompleted', [tag]);
					}
		...
		Lua_helper.add_callback(lua, "runTimer", function(tag:String, time:Float = 1, loops:Int = 1) {
			cancelTimer(tag);
			MainMenuState.instance.modchartTimers.set(tag, new FlxTimer().start(time, function(tmr:FlxTimer) {
				if(tmr.finished) {
					MainMenuState.instance.modchartTimers.remove(tag);
				}
				MainMenuState.instance.callOnLuas('onTimerCompleted', [tag, tmr.loops, tmr.loopsLeft]);
		...
		Lua_helper.add_callback(lua, "makeLuaSprite", function(tag:String, image:String, x:Float, y:Float) {
			tag = tag.replace('.', '');
			resetSpriteTag(tag);
			var leSprite:MenuModchartSprite = new MenuModchartSprite(x, y);
			if(image != null && image.length > 0)
			{
				leSprite.loadGraphic(Paths.image(image));
			}
			leSprite.antialiasing = ClientPrefs.globalAntialiasing;
			MainMenuState.instance.modchartSprites.set(tag, leSprite);
		...
		Lua_helper.add_callback(lua, "makeGraphic", function(obj:String, width:Int, height:Int, color:String) {
			...
			var spr:FlxSprite = MainMenuState.instance.getLuaObject(obj,false);
			if(spr!=null) {
				MainMenuState.instance.getLuaObject(obj,false).makeGraphic(width, height, colorNum);
				return;
			}
		...
		Lua_helper.add_callback(lua, "addAnimationByPrefix", function(obj:String, name:String, prefix:String, framerate:Int = 24, loop:Bool = true) {
			if(MainMenuState.instance.getLuaObject(obj,false)!=null) {
				var cock:FlxSprite = MainMenuState.instance.getLuaObject(obj,false);
		...
		{
			if(MainMenuState.instance.getLuaObject(obj, false) != null) {
				var luaObj:FlxSprite = MainMenuState.instance.getLuaObject(obj,false);
				if(luaObj.animation.getByName(name) != null)
				{
					luaObj.animation.play(name, forced, reverse, startFrame);
					if(Std.isOfType(luaObj, MenuModchartSprite))
					{
						//convert luaObj to ModchartSprite
						var obj:Dynamic = luaObj;
						var luaObj:MenuModchartSprite = obj;
		...
		Lua_helper.add_callback(lua, "addOffset", function(obj:String, anim:String, x:Float, y:Float) {
			if(MainMenuState.instance.modchartSprites.exists(obj)) {
				MainMenuState.instance.modchartSprites.get(obj).animOffsets.set(anim, [x, y]);
				return true;
			}
		...
		Lua_helper.add_callback(lua, "setScrollFactor", function(obj:String, scrollX:Float, scrollY:Float) {
			if(MainMenuState.instance.getLuaObject(obj,false)!=null) {
				MainMenuState.instance.getLuaObject(obj,false).scrollFactor.set(scrollX, scrollY);
				return;
			}
		...
		Lua_helper.add_callback(lua, "addLuaSprite", function(tag:String, front:Bool = false) {
			if(MainMenuState.instance.modchartSprites.exists(tag)) {
				var shit:MenuModchartSprite = MainMenuState.instance.modchartSprites.get(tag);
				if(!shit.wasAdded) {
					if(front)
					{
						MainMenuState.instance.add(shit);
					}
					else
						{
							var position:Int = MainMenuState.instance.members.indexOf(MainMenuState.instance.bg);
							MainMenuState.instance.insert(position + 1, shit);
						}
					shit.wasAdded = true;
		...
		Lua_helper.add_callback(lua, "setGraphicSize", function(obj:String, x:Int, y:Int = 0, updateHitbox:Bool = true) {
			if(MainMenuState.instance.getLuaObject(obj)!=null) {
				var shit:FlxSprite = MainMenuState.instance.getLuaObject(obj);
		...
		Lua_helper.add_callback(lua, "removeLuaSprite", function(tag:String, destroy:Bool = true) {
			...
			var pee:MenuModchartSprite = MainMenuState.instance.modchartSprites.get(tag);
			if(destroy) {
				pee.kill();
			}
			...
			if(destroy) {
				pee.destroy();
				MainMenuState.instance.modchartSprites.remove(tag);
			}
		});

		Lua_helper.add_callback(lua, "luaSpriteExists", function(tag:String) {
			return MainMenuState.instance.modchartSprites.exists(tag);
		});
		Lua_helper.add_callback(lua, "luaTextExists", function(tag:String) {
			return MainMenuState.instance.modchartTexts.exists(tag);
		});
		Lua_helper.add_callback(lua, "luaSoundExists", function(tag:String) {
			return MainMenuState.instance.modchartSounds.exists(tag);
		});


		Lua_helper.add_callback(lua, "setObjectCamera", function(obj:String, camera:String = '') {
			var real = MainMenuState.instance.getLuaObject(obj);
			if(real!=null){
		...
		Lua_helper.add_callback(lua, "screenCenter", function(obj:String, pos:String = 'xy') {
			var spr:FlxSprite = MainMenuState.instance.getLuaObject(obj);
		...
		Lua_helper.add_callback(lua, "objectsOverlap", function(obj1:String, obj2:String) {
			...
			{
				var real = MainMenuState.instance.getLuaObject(namesArray[i]);
				if(real!=null) {
		...
		Lua_helper.add_callback(lua, "playSound", function(sound:String, volume:Float = 1, ?tag:String = null) {
			if(tag != null && tag.length > 0) {
				tag = tag.replace('.', '');
				if(MainMenuState.instance.modchartSounds.exists(tag)) {
					MainMenuState.instance.modchartSounds.get(tag).stop();
				}
				MainMenuState.instance.modchartSounds.set(tag, FlxG.sound.play(Paths.sound(sound), volume, false, function() {
					MainMenuState.instance.modchartSounds.remove(tag);
					MainMenuState.instance.callOnLuas('onSoundFinished', [tag]);
				}));
				return;
			}
		...
		Lua_helper.add_callback(lua, "stopSound", function(tag:String) {
			if(tag != null && tag.length > 1 && MainMenuState.instance.modchartSounds.exists(tag)) {
				MainMenuState.instance.modchartSounds.get(tag).stop();
				MainMenuState.instance.modchartSounds.remove(tag);
			}
		});
		Lua_helper.add_callback(lua, "pauseSound", function(tag:String) {
			if(tag != null && tag.length > 1 && MainMenuState.instance.modchartSounds.exists(tag)) {
				MainMenuState.instance.modchartSounds.get(tag).pause();
			}
		});
		Lua_helper.add_callback(lua, "resumeSound", function(tag:String) {
			if(tag != null && tag.length > 1 && MainMenuState.instance.modchartSounds.exists(tag)) {
				MainMenuState.instance.modchartSounds.get(tag).play();
			}
		});
		Lua_helper.add_callback(lua, "soundFadeIn", function(tag:String, duration:Float, fromValue:Float = 0, toValue:Float = 1) {
			...
			} else if(MainMenuState.instance.modchartSounds.exists(tag)) {
				MainMenuState.instance.modchartSounds.get(tag).fadeIn(duration, fromValue, toValue);
			}
		...
		Lua_helper.add_callback(lua, "soundFadeOut", function(tag:String, duration:Float, toValue:Float = 0) {
			if(tag == null || tag.length < 1) {
				...
				MainMenuState.instance.modchartSounds.get(tag).fadeOut(duration, toValue);
			}
		});
		Lua_helper.add_callback(lua, "soundFadeCancel", function(tag:String) {
			if(tag == null || tag.length < 1) {
				...
				var theSound:FlxSound = MainMenuState.instance.modchartSounds.get(tag);
		...
		Lua_helper.add_callback(lua, "getSoundVolume", function(tag:String) {
			if(tag == null || tag.length < 1) {
				...
				return MainMenuState.instance.modchartSounds.get(tag).volume;
			}
			return 0;
		});
		Lua_helper.add_callback(lua, "setSoundVolume", function(tag:String, value:Float) {
			if(tag == null || tag.length < 1) {
				...
				MainMenuState.instance.modchartSounds.get(tag).volume = value;
			}
		});
		Lua_helper.add_callback(lua, "getSoundTime", function(tag:String) {
			if(tag != null && tag.length > 0 && MainMenuState.instance.modchartSounds.exists(tag)) {
				return MainMenuState.instance.modchartSounds.get(tag).time;
			}
			return 0;
		});
		...
		Lua_helper.add_callback(lua, "makeLuaText", function(tag:String, text:String, width:Int, x:Float, y:Float) {
			tag = tag.replace('.', '');
			resetTextTag(tag);
			var leText:MenuModchartText = new MenuModchartText(x, y, text, width);
			MainMenuState.instance.modchartTexts.set(tag, leText);
		});
		...
		Lua_helper.add_callback(lua, "addLuaText", function(tag:String) {
			if(MainMenuState.instance.modchartTexts.exists(tag)) {
				var shit:MenuModchartText = MainMenuState.instance.modchartTexts.get(tag);
				if(!shit.wasAdded) {
		...
		Lua_helper.add_callback(lua, "removeLuaText", function(tag:String, destroy:Bool = true) {
			...
			var pee:MenuModchartText = MainMenuState.instance.modchartTexts.get(tag);
			if(destroy) {
				pee.kill();
			}
			...
			if(destroy) {
				pee.destroy();
				MainMenuState.instance.modchartTexts.remove(tag);
			}
		});

		Lua_helper.add_callback(lua, "initSaveData", function(name:String, ?folder:String = 'psychenginemods') {
			if(!MainMenuState.instance.modchartSaves.exists(name))
			{
				var save:FlxSave = new FlxSave();
				save.bind(name, folder);
				MainMenuState.instance.modchartSaves.set(name, save);
				return;
			}
		...
		Lua_helper.add_callback(lua, "flushSaveData", function(name:String) {
			if(MainMenuState.instance.modchartSaves.exists(name))
			{
				MainMenuState.instance.modchartSaves.get(name).flush();
				return;
			}
		...
		Lua_helper.add_callback(lua, "getDataFromSave", function(name:String, field:String, ?defaultValue:Dynamic = null) {
			if(MainMenuState.instance.modchartSaves.exists(name))
			{
				var retVal:Dynamic = Reflect.field(MainMenuState.instance.modchartSaves.get(name).data, field);
				return retVal;
			}
		...
		Lua_helper.add_callback(lua, "setDataFromSave", function(name:String, field:String, value:Dynamic) {
			if(MainMenuState.instance.modchartSaves.exists(name))
			{
				Reflect.setField(MainMenuState.instance.modchartSaves.get(name).data, field, value);
				return;
			}
		...
		Lua_helper.add_callback(lua, "luaSpriteMakeGraphic", function(tag:String, width:Int, height:Int, color:String) {
			...
			if(MainMenuState.instance.modchartSprites.exists(tag)) {
				...
				MainMenuState.instance.modchartSprites.get(tag).makeGraphic(width, height, colorNum);
			}
		});
		Lua_helper.add_callback(lua, "luaSpriteAddAnimationByPrefix", function(tag:String, name:String, prefix:String, framerate:Int = 24, loop:Bool = true) {
			...
			if(MainMenuState.instance.modchartSprites.exists(tag)) {
				var cock:MenuModchartSprite = MainMenuState.instance.modchartSprites.get(tag);
				cock.animation.addByPrefix(name, prefix, framerate, loop);
				if(cock.animation.curAnim == null) {
					cock.animation.play(name, true);
				}
		...
		Lua_helper.add_callback(lua, "luaSpriteAddAnimationByIndices", function(tag:String, name:String, prefix:String, indices:String, framerate:Int = 24) {
			...
			if(MainMenuState.instance.modchartSprites.exists(tag)) {
				...
				var pussy:MenuModchartSprite = MainMenuState.instance.modchartSprites.get(tag);
		...
		Lua_helper.add_callback(lua, "setLuaSpriteCamera", function(tag:String, camera:String = '') {
			...
			if(MainMenuState.instance.modchartSprites.exists(tag)) {
				MainMenuState.instance.modchartSprites.get(tag).cameras = [cameraFromString(camera)];
				return true;
			}
		...
		Lua_helper.add_callback(lua, "getPropertyLuaSprite", function(tag:String, variable:String) {
			...
			if(MainMenuState.instance.modchartSprites.exists(tag)) {
				var killMe:Array<String> = variable.split('.');
				if(killMe.length > 1) {
					var coverMeInPiss:Dynamic = Reflect.getProperty(MainMenuState.instance.modchartSprites.get(tag), killMe[0]);
					for (i in 1...killMe.length-1) {
	...
	public function initHaxeModule()
	{
		if(hscript == null)
		{
			...
			hscript = new MenuHScript(); //TO DO: Fix issue with 2 scripts not being able to use the same variable names
		}
	}
	#end
	...
	public static function setVarInArray(instance:Dynamic, variable:String, value:Dynamic):Any
	{
		...
		if(shit.length > 1)
		{
			...
			if(MainMenuState.instance.variables.exists(shit[0]))
			{
				var retVal:Dynamic = MainMenuState.instance.variables.get(shit[0]);
				if(retVal != null)
					blah = retVal;
			}
			else
		...
		if(MainMenuState.instance.variables.exists(variable))
		{
			MainMenuState.instance.variables.set(variable, value);
			return true;
		}
	...
	public static function getVarInArray(instance:Dynamic, variable:String):Any
	{
		...
		if(MainMenuState.instance.variables.exists(variable))
		{
			var retVal:Dynamic = MainMenuState.instance.variables.get(variable);
			if(retVal != null)
				return retVal;
		}
	...
	{
		return MainMenuState.instance.modchartTexts.exists(name) ? MainMenuState.instance.modchartTexts.get(name) : Reflect.getProperty(MainMenuState.instance, name);
	}
	...
	function initLuaShader(name:String, ?glslVersion:Int = 120)
	{
		...
		#if (!flash && sys)
		if(MainMenuState.instance.runtimeShaders.exists(name))
		{
		...
		for (folder in foldersToCheck)
		{
			if(FileSystem.exists(folder))
			{
				...
				if(found)
				{
					MainMenuState.instance.runtimeShaders.set(name, [frag, vert]);
	...
	function resetTextTag(tag:String) {
		...
		pee.kill();
		if(pee.wasAdded) {
			MainMenuState.instance.remove(pee, true);
		}
		pee.destroy();
		MainMenuState.instance.modchartTexts.remove(tag);
	}
	...
	function cancelTween(tag:String) {
		if(MainMenuState.instance.modchartTweens.exists(tag)) {
			MainMenuState.instance.modchartTweens.get(tag).cancel();
			MainMenuState.instance.modchartTweens.get(tag).destroy();
			MainMenuState.instance.modchartTweens.remove(tag);
		}
	}
	...
	function cancelTimer(tag:String) {
		if(MainMenuState.instance.modchartTimers.exists(tag)) {
			var theTimer:FlxTimer = MainMenuState.instance.modchartTimers.get(tag);
			theTimer.cancel();
			theTimer.destroy();
			MainMenuState.instance.modchartTimers.remove(tag);
		}
	...
	function cameraFromString(cam:String):FlxCamera {
		switch(cam.toLowerCase()) {
			case 'camhud' | 'hud': return MainMenuState.instance.camHUD;
			case 'camother' | 'other': return MainMenuState.instance.camOther;
		}
		return MainMenuState.instance.camGame;
	}
	...
	static function addAnimByIndices(obj:String, name:String, prefix:String, indices:String, framerate:Int = 24, loop:Bool = false)
	{
		...
		if(MainMenuState.instance.getLuaObject(obj, false)!=null) {
			var pussy:FlxSprite = MainMenuState.instance.getLuaObject(obj, false);
	...
	{
		var coverMeInPiss:Dynamic = MainMenuState.instance.getLuaObject(objectName, checkForTextsToo);
...
class MenuModchartSprite extends FlxSprite
{
...
class MenuModchartText extends FlxText
{
	public var wasAdded:Bool = false;
	public function new(x:Float, y:Float, text:String, width:Float)
	{
		...
		cameras = [MainMenuState.instance.camHUD];
		scrollFactor.set();
		borderSize = 2;
	}
}

class MenuDebugLuaText extends FlxText
{
	private var disableTime:Float = 6;
	public var parentGroup:FlxTypedGroup<MenuDebugLuaText>;
	public function new(text:String, parentGroup:FlxTypedGroup<MenuDebugLuaText>, color:FlxColor) {
...
class MenuCustomSubstate extends MusicBeatSubstate
{
	public static var name:String = 'unnamed';
	public static var instance:MenuCustomSubstate;

	override function create()
	{
		instance = this;

		MainMenuState.instance.callOnLuas('onCustomSubstateCreate', [name]);
		super.create();
		MainMenuState.instance.callOnLuas('onCustomSubstateCreatePost', [name]);
	}
	
	public function new(name:String)
	{
		MenuCustomSubstate.name = name;
		super();
	...
	override function update(elapsed:Float)
	{
		MainMenuState.instance.callOnLuas('onCustomSubstateUpdate', [name, elapsed]);
		super.update(elapsed);
		MainMenuState.instance.callOnLuas('onCustomSubstateUpdatePost', [name, elapsed]);
	}

	override function destroy()
	{
		MainMenuState.instance.callOnLuas('onCustomSubstateDestroy', [name]);
		super.destroy();
	}
}
...
class MenuHScript
{
	...
	public function new()
	{
		...
		interp.variables.set('game', MainMenuState.instance);
		interp.variables.set('Paths', Paths);
		interp.variables.set('Conductor', Conductor);
		interp.variables.set('ClientPrefs', ClientPrefs);
		interp.variables.set('Character', Character);
		interp.variables.set('Alphabet', Alphabet);
		interp.variables.set('CustomSubstate', MenuCustomSubstate);
		#if (!flash && sys)
	...
	{
		@:privateAccess
		MenuHScript.parser.line = 1;
		MenuHScript.parser.allowTypes = true;
		return interp.execute(MenuHScript.parser.parseString(codeToRun));
	}
}
#end
