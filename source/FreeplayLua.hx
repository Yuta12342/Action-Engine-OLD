...
class FreeplayLua {
	...
	public function new(script:String) {
		...
		//set('bpm', FreeplayState.SONG.bpm);
		//set('scrollSpeed', FreeplayState.SONG.speed);
		set('crochet', Conductor.crochet);
		set('stepCrochet', Conductor.stepCrochet);
		set('songLength', FlxG.sound.music.length);
		//set('songName', FreeplayState.SONG.song);
		//set('songPath', Paths.formatToSongPath(FreeplayState.SONG.song));
		set('startedCountdown', false);
		//set('mania', FreeplayState.mania);
		//set('curStage', FreeplayState.SONG.stage);

		//set('isStoryMode', FreeplayState.isStoryMode);
		//set('difficulty', FreeplayState.storyDifficulty);

		//var difficultyName:String = CoolUtil.difficulties[FreeplayState.storyDifficulty];
		//set('difficultyName', difficultyName);
		//set('difficultyPath', Paths.formatToSongPath(difficultyName));
		//set('weekRaw', FreeplayState.storyWeek);
		//set('week', WeekData.weeksList[FreeplayState.storyWeek]);
		//set('seenCutscene', FreeplayState.seenCutscene);
		...
		// FreeplayState cringe ass nae nae bullcrap
		set('curBeat', 0);
		set('curStep', 0);
		set('curDecBeat', 0);
		set('curDecStep', 0);
		...
		set('ratingName', '');
		set('ratingFC', '');
		//set('version', FreeplayState.psychEngineVersion.trim());
		...
		// Gameplay settings
		//set('healthGainMult', FreeplayState.instance.healthGain);
		//set('healthLossMult', FreeplayState.instance.healthLoss);
		//set('playbackRate', FreeplayState.instance.playbackRate);
		//set('instakillOnMiss', FreeplayState.instance.instakillOnMiss);
		//set('botPlay', FreeplayState.instance.cpuControlled);
		//set('practice', FreeplayState.instance.practiceMode);

		/*for (i in 0...FreeplayState.mania) {
		...
		//set('defaultBoyfriendX', FreeplayState.instance.BF_X);
		//set('defaultBoyfriendY', FreeplayState.instance.BF_Y);
		//set('defaultOpponentX', FreeplayState.instance.DAD_X);
		//set('defaultOpponentY', FreeplayState.instance.DAD_Y);
		//set('defaultGirlfriendX', FreeplayState.instance.GF_X);
		//set('defaultGirlfriendY', FreeplayState.instance.GF_Y);

		// Character shit
		//set('boyfriendName', FreeplayState.SONG.player1);
		//set('dadName', FreeplayState.SONG.player2);
		//set('gfName', FreeplayState.SONG.gfVersion);
		...
		Lua_helper.add_callback(lua, "openCustomSubstate", function(name:String, pauseGame:Bool = false) {
			if(pauseGame)
			{
				...
				if(FlxG.sound.music != null) {
					...
					FreeplayState.instance.vocals.pause();
				}*/
			}
			FreeplayState.instance.openSubState(new FreeplayCustomSubstate(name));
		});

		Lua_helper.add_callback(lua, "closeCustomSubstate", function() {
			if(FreeplayCustomSubstate.instance != null)
			{
				FreeplayState.instance.closeSubState();
				FreeplayCustomSubstate.instance = null;
				return true;
			}
			return false;
		});
		...
		Lua_helper.add_callback(lua, "setSpriteShader", function(obj:String, shader:String) {
			...
			#if (!flash && MODS_ALLOWED && sys)
			if(!FreeplayState.instance.runtimeShaders.exists(shader) && !initLuaShader(shader))
			{
			...
			if(leObj != null) {
				var arr:Array<String> = FreeplayState.instance.runtimeShaders.get(shader);
		...
		Lua_helper.add_callback(lua, "getRunningScripts", function(){
			...
			for (idx in 0...FreeplayState.instance.luaArray.length)
				runningScripts.push(FreeplayState.instance.luaArray[idx].scriptName);


			return runningScripts;
		});
		/*Lua_helper.add_callback(lua, "addSongToFreeplay", function(songName:String, weekNum:Int, songCharacter:String, colors:Array<Int>){
			var colors:Array<Int> = colors;
			if(colors == null || colors.length < 3)
			{
				colors = [146, 113, 253];
			}
			FreeplayState.songs.push(new FreeplayState.SongMetadata(songName, weekNum, songCharacter, FlxColor.fromRGB(colors[0], colors[1], colors[2])));
		});*/
		...
		Lua_helper.add_callback(lua, "callOnLuas", function(?funcName:String, ?args:Array<Dynamic>, ignoreStops=false, ignoreSelf=true, ?exclusions:Array<String>){
			...
			FreeplayState.instance.callOnLuas(funcName, args, ignoreStops, exclusions);
		});
		...
		Lua_helper.add_callback(lua, "callScript", function(?luaFile:String, ?funcName:String, ?args:Array<Dynamic>){
			...
			if(doPush)
			{
				for (luaInstance in FreeplayState.instance.luaArray)
				{
		...
		Lua_helper.add_callback(lua, "addLuaScript", function(luaFile:String, ?ignoreAlreadyRunning:Bool = false) { //would be dope asf.
			...
			if(doPush)
			{
				if(!ignoreAlreadyRunning)
				{
					...
					FreeplayState.instance.luaArray.push(new FreeplayLua(cervix));
					return;
				}
				...
				if(!ignoreAlreadyRunning)
				{
					for (luaInstance in FreeplayState.instance.luaArray)
					{
						if(luaInstance.scriptName == cervix)
						{
							...
							FreeplayState.instance.luaArray.remove(luaInstance);
							return;
						}
					}
				}
		...
		Lua_helper.add_callback(lua, "loadSong", function(?name:String = null, ?difficultyNum:Int = -1) {
			...
			if (difficultyNum == -1)
			luaTrace('Unrecommended Action in Freeplay: -1 on diffNum.');
				difficultyNum = PlayState.storyDifficulty;
				throw ('Invalid Variable on Song Difficulty in function: "loadSong", while on FreeplayState');
		...
		Lua_helper.add_callback(lua, "doTweenX", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			...
			if(penisExam != null) {
				FreeplayState.instance.modchartTweens.set(tag, FlxTween.tween(penisExam, {x: value}, duration, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						FreeplayState.instance.callOnLuas('onTweenCompleted', [tag]);
						FreeplayState.instance.modchartTweens.remove(tag);
					}
				}));
			} else {
		...
		Lua_helper.add_callback(lua, "doTweenY", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
				FreeplayState.instance.modchartTweens.set(tag, FlxTween.tween(penisExam, {y: value}, duration, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						FreeplayState.instance.callOnLuas('onTweenCompleted', [tag]);
						FreeplayState.instance.modchartTweens.remove(tag);
					}
		...
		Lua_helper.add_callback(lua, "doTweenAngle", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
				FreeplayState.instance.modchartTweens.set(tag, FlxTween.tween(penisExam, {angle: value}, duration, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						FreeplayState.instance.callOnLuas('onTweenCompleted', [tag]);
						FreeplayState.instance.modchartTweens.remove(tag);
					}
		...
		Lua_helper.add_callback(lua, "doTweenAlpha", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
				FreeplayState.instance.modchartTweens.set(tag, FlxTween.tween(penisExam, {alpha: value}, duration, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						FreeplayState.instance.callOnLuas('onTweenCompleted', [tag]);
						FreeplayState.instance.modchartTweens.remove(tag);
					}
		...
		Lua_helper.add_callback(lua, "doTweenZoom", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
				FreeplayState.instance.modchartTweens.set(tag, FlxTween.tween(penisExam, {zoom: value}, duration, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						FreeplayState.instance.callOnLuas('onTweenCompleted', [tag]);
						FreeplayState.instance.modchartTweens.remove(tag);
					}
		...
		Lua_helper.add_callback(lua, "doTweenColor", function(tag:String, vars:String, targetColor:String, duration:Float, ease:String) {
			...
			if(penisExam != null) {
				...
				FreeplayState.instance.modchartTweens.set(tag, FlxTween.color(penisExam, duration, curColor, color, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						FreeplayState.instance.modchartTweens.remove(tag);
						FreeplayState.instance.callOnLuas('onTweenCompleted', [tag]);
					}
		...
		Lua_helper.add_callback(lua, "runTimer", function(tag:String, time:Float = 1, loops:Int = 1) {
			cancelTimer(tag);
			FreeplayState.instance.modchartTimers.set(tag, new FlxTimer().start(time, function(tmr:FlxTimer) {
				if(tmr.finished) {
					FreeplayState.instance.modchartTimers.remove(tag);
				}
				FreeplayState.instance.callOnLuas('onTimerCompleted', [tag, tmr.loops, tmr.loopsLeft]);
		...
		Lua_helper.add_callback(lua, "makeLuaSprite", function(tag:String, image:String, x:Float, y:Float) {
			tag = tag.replace('.', '');
			resetSpriteTag(tag);
			var leSprite:FreeplayModchartSprite = new FreeplayModchartSprite(x, y);
			if(image != null && image.length > 0)
			{
				leSprite.loadGraphic(Paths.image(image));
			}
			leSprite.antialiasing = ClientPrefs.globalAntialiasing;
			FreeplayState.instance.modchartSprites.set(tag, leSprite);
		...
		Lua_helper.add_callback(lua, "makeGraphic", function(obj:String, width:Int, height:Int, color:String) {
			...
			var spr:FlxSprite = FreeplayState.instance.getLuaObject(obj,false);
			if(spr!=null) {
				FreeplayState.instance.getLuaObject(obj,false).makeGraphic(width, height, colorNum);
				return;
			}
		...
		Lua_helper.add_callback(lua, "addAnimationByPrefix", function(obj:String, name:String, prefix:String, framerate:Int = 24, loop:Bool = true) {
			if(FreeplayState.instance.getLuaObject(obj,false)!=null) {
				var cock:FlxSprite = FreeplayState.instance.getLuaObject(obj,false);
		...
		{
			if(FreeplayState.instance.getLuaObject(obj, false) != null) {
				var luaObj:FlxSprite = FreeplayState.instance.getLuaObject(obj,false);
				if(luaObj.animation.getByName(name) != null)
				{
					luaObj.animation.play(name, forced, reverse, startFrame);
					if(Std.isOfType(luaObj, FreeplayModchartSprite))
					{
						//convert luaObj to ModchartSprite
						var obj:Dynamic = luaObj;
						var luaObj:FreeplayModchartSprite = obj;
		...
		Lua_helper.add_callback(lua, "addOffset", function(obj:String, anim:String, x:Float, y:Float) {
			if(FreeplayState.instance.modchartSprites.exists(obj)) {
				FreeplayState.instance.modchartSprites.get(obj).animOffsets.set(anim, [x, y]);
				return true;
			}
		...
		Lua_helper.add_callback(lua, "setScrollFactor", function(obj:String, scrollX:Float, scrollY:Float) {
			if(FreeplayState.instance.getLuaObject(obj,false)!=null) {
				FreeplayState.instance.getLuaObject(obj,false).scrollFactor.set(scrollX, scrollY);
				return;
			}
		...
		Lua_helper.add_callback(lua, "addLuaSprite", function(tag:String, front:Bool = false) {
			if(FreeplayState.instance.modchartSprites.exists(tag)) {
				var shit:FreeplayModchartSprite = FreeplayState.instance.modchartSprites.get(tag);
				if(!shit.wasAdded) {
					if(front)
					{
						FreeplayState.instance.add(shit);
					}
					else
						{
							var position:Int = FreeplayState.instance.members.indexOf(FreeplayState.instance.bg);
							FreeplayState.instance.insert(position + 1, shit);
						}
					shit.wasAdded = true;
		...
		Lua_helper.add_callback(lua, "setGraphicSize", function(obj:String, x:Int, y:Int = 0, updateHitbox:Bool = true) {
			if(FreeplayState.instance.getLuaObject(obj)!=null) {
				var shit:FlxSprite = FreeplayState.instance.getLuaObject(obj);
		...
		Lua_helper.add_callback(lua, "removeLuaSprite", function(tag:String, destroy:Bool = true) {
			...
			var pee:FreeplayModchartSprite = FreeplayState.instance.modchartSprites.get(tag);
			if(destroy) {
				pee.kill();
			}
			...
			if(destroy) {
				pee.destroy();
				FreeplayState.instance.modchartSprites.remove(tag);
			}
		});

		Lua_helper.add_callback(lua, "luaSpriteExists", function(tag:String) {
			return FreeplayState.instance.modchartSprites.exists(tag);
		});
		Lua_helper.add_callback(lua, "luaTextExists", function(tag:String) {
			return FreeplayState.instance.modchartTexts.exists(tag);
		});
		Lua_helper.add_callback(lua, "luaSoundExists", function(tag:String) {
			return FreeplayState.instance.modchartSounds.exists(tag);
		});


		Lua_helper.add_callback(lua, "setObjectCamera", function(obj:String, camera:String = '') {
			var real = FreeplayState.instance.getLuaObject(obj);
			if(real!=null){
		...
		Lua_helper.add_callback(lua, "screenCenter", function(obj:String, pos:String = 'xy') {
			var spr:FlxSprite = FreeplayState.instance.getLuaObject(obj);
		...
		Lua_helper.add_callback(lua, "objectsOverlap", function(obj1:String, obj2:String) {
			...
			{
				var real = FreeplayState.instance.getLuaObject(namesArray[i]);
				if(real!=null) {
		...
		Lua_helper.add_callback(lua, "playSound", function(sound:String, volume:Float = 1, ?tag:String = null) {
			if(tag != null && tag.length > 0) {
				tag = tag.replace('.', '');
				if(FreeplayState.instance.modchartSounds.exists(tag)) {
					FreeplayState.instance.modchartSounds.get(tag).stop();
				}
				FreeplayState.instance.modchartSounds.set(tag, FlxG.sound.play(Paths.sound(sound), volume, false, function() {
					FreeplayState.instance.modchartSounds.remove(tag);
					FreeplayState.instance.callOnLuas('onSoundFinished', [tag]);
				}));
				return;
			}
		...
		Lua_helper.add_callback(lua, "stopSound", function(tag:String) {
			if(tag != null && tag.length > 1 && FreeplayState.instance.modchartSounds.exists(tag)) {
				FreeplayState.instance.modchartSounds.get(tag).stop();
				FreeplayState.instance.modchartSounds.remove(tag);
			}
		});
		Lua_helper.add_callback(lua, "pauseSound", function(tag:String) {
			if(tag != null && tag.length > 1 && FreeplayState.instance.modchartSounds.exists(tag)) {
				FreeplayState.instance.modchartSounds.get(tag).pause();
			}
		});
		Lua_helper.add_callback(lua, "resumeSound", function(tag:String) {
			if(tag != null && tag.length > 1 && FreeplayState.instance.modchartSounds.exists(tag)) {
				FreeplayState.instance.modchartSounds.get(tag).play();
			}
		});
		Lua_helper.add_callback(lua, "soundFadeIn", function(tag:String, duration:Float, fromValue:Float = 0, toValue:Float = 1) {
			...
			} else if(FreeplayState.instance.modchartSounds.exists(tag)) {
				FreeplayState.instance.modchartSounds.get(tag).fadeIn(duration, fromValue, toValue);
			}
		...
		Lua_helper.add_callback(lua, "soundFadeOut", function(tag:String, duration:Float, toValue:Float = 0) {
			if(tag == null || tag.length < 1) {
				...
				FreeplayState.instance.modchartSounds.get(tag).fadeOut(duration, toValue);
			}
		});
		Lua_helper.add_callback(lua, "soundFadeCancel", function(tag:String) {
			if(tag == null || tag.length < 1) {
				...
				var theSound:FlxSound = FreeplayState.instance.modchartSounds.get(tag);
		...
		Lua_helper.add_callback(lua, "getSoundVolume", function(tag:String) {
			if(tag == null || tag.length < 1) {
				...
				return FreeplayState.instance.modchartSounds.get(tag).volume;
			}
			return 0;
		});
		Lua_helper.add_callback(lua, "setSoundVolume", function(tag:String, value:Float) {
			if(tag == null || tag.length < 1) {
				...
				FreeplayState.instance.modchartSounds.get(tag).volume = value;
			}
		});
		Lua_helper.add_callback(lua, "getSoundTime", function(tag:String) {
			if(tag != null && tag.length > 0 && FreeplayState.instance.modchartSounds.exists(tag)) {
				return FreeplayState.instance.modchartSounds.get(tag).time;
			}
			return 0;
		});
		...
		Lua_helper.add_callback(lua, "makeLuaText", function(tag:String, text:String, width:Int, x:Float, y:Float) {
			tag = tag.replace('.', '');
			resetTextTag(tag);
			var leText:FreeplayModchartText = new FreeplayModchartText(x, y, text, width);
			FreeplayState.instance.modchartTexts.set(tag, leText);
		});
		...
		Lua_helper.add_callback(lua, "addLuaText", function(tag:String) {
			if(FreeplayState.instance.modchartTexts.exists(tag)) {
				var shit:FreeplayModchartText = FreeplayState.instance.modchartTexts.get(tag);
				if(!shit.wasAdded) {
		...
		Lua_helper.add_callback(lua, "removeLuaText", function(tag:String, destroy:Bool = true) {
			...
			var pee:FreeplayModchartText = FreeplayState.instance.modchartTexts.get(tag);
			if(destroy) {
				pee.kill();
			}
			...
			if(destroy) {
				pee.destroy();
				FreeplayState.instance.modchartTexts.remove(tag);
			}
		});

		Lua_helper.add_callback(lua, "initSaveData", function(name:String, ?folder:String = 'psychenginemods') {
			if(!FreeplayState.instance.modchartSaves.exists(name))
			{
				var save:FlxSave = new FlxSave();
				save.bind(name, folder);
				FreeplayState.instance.modchartSaves.set(name, save);
				return;
			}
		...
		Lua_helper.add_callback(lua, "flushSaveData", function(name:String) {
			if(FreeplayState.instance.modchartSaves.exists(name))
			{
				FreeplayState.instance.modchartSaves.get(name).flush();
				return;
			}
		...
		Lua_helper.add_callback(lua, "getDataFromSave", function(name:String, field:String, ?defaultValue:Dynamic = null) {
			if(FreeplayState.instance.modchartSaves.exists(name))
			{
				var retVal:Dynamic = Reflect.field(FreeplayState.instance.modchartSaves.get(name).data, field);
				return retVal;
			}
		...
		Lua_helper.add_callback(lua, "setDataFromSave", function(name:String, field:String, value:Dynamic) {
			if(FreeplayState.instance.modchartSaves.exists(name))
			{
				Reflect.setField(FreeplayState.instance.modchartSaves.get(name).data, field, value);
				return;
			}
		...
		Lua_helper.add_callback(lua, "luaSpriteMakeGraphic", function(tag:String, width:Int, height:Int, color:String) {
			...
			if(FreeplayState.instance.modchartSprites.exists(tag)) {
				...
				FreeplayState.instance.modchartSprites.get(tag).makeGraphic(width, height, colorNum);
			}
		});
		Lua_helper.add_callback(lua, "luaSpriteAddAnimationByPrefix", function(tag:String, name:String, prefix:String, framerate:Int = 24, loop:Bool = true) {
			...
			if(FreeplayState.instance.modchartSprites.exists(tag)) {
				var cock:FreeplayModchartSprite = FreeplayState.instance.modchartSprites.get(tag);
				cock.animation.addByPrefix(name, prefix, framerate, loop);
				if(cock.animation.curAnim == null) {
					cock.animation.play(name, true);
				}
		...
		Lua_helper.add_callback(lua, "luaSpriteAddAnimationByIndices", function(tag:String, name:String, prefix:String, indices:String, framerate:Int = 24) {
			...
			if(FreeplayState.instance.modchartSprites.exists(tag)) {
				...
				var pussy:FreeplayModchartSprite = FreeplayState.instance.modchartSprites.get(tag);
		...
		Lua_helper.add_callback(lua, "setLuaSpriteCamera", function(tag:String, camera:String = '') {
			...
			if(FreeplayState.instance.modchartSprites.exists(tag)) {
				FreeplayState.instance.modchartSprites.get(tag).cameras = [cameraFromString(camera)];
				return true;
			}
		...
		Lua_helper.add_callback(lua, "getPropertyLuaSprite", function(tag:String, variable:String) {
			...
			if(FreeplayState.instance.modchartSprites.exists(tag)) {
				var killMe:Array<String> = variable.split('.');
				if(killMe.length > 1) {
					var coverMeInPiss:Dynamic = Reflect.getProperty(FreeplayState.instance.modchartSprites.get(tag), killMe[0]);
					for (i in 1...killMe.length-1) {
	...
	public function initHaxeModule()
	{
		if(hscript == null)
		{
			...
			hscript = new FreeplayHScript(); //TO DO: Fix issue with 2 scripts not being able to use the same variable names
		}
	}
	#end
	...
	public static function setVarInArray(instance:Dynamic, variable:String, value:Dynamic):Any
	{
		...
		if(shit.length > 1)
		{
			...
			if(FreeplayState.instance.variables.exists(shit[0]))
			{
				var retVal:Dynamic = FreeplayState.instance.variables.get(shit[0]);
				if(retVal != null)
					blah = retVal;
			}
			else
	...
	public static function getVarInArray(instance:Dynamic, variable:String):Any
	{
		...
		if(FreeplayState.instance.variables.exists(variable))
		{
			var retVal:Dynamic = FreeplayState.instance.variables.get(variable);
			if(retVal != null)
				return retVal;
		}
	...
	{
		return FreeplayState.instance.modchartTexts.exists(name) ? FreeplayState.instance.modchartTexts.get(name) : Reflect.getProperty(FreeplayState.instance, name);
	}
	...
	function initLuaShader(name:String, ?glslVersion:Int = 120)
	{
		...
		#if (!flash && sys)
		if(FreeplayState.instance.runtimeShaders.exists(name))
		{
		...
		for (folder in foldersToCheck)
		{
			if(FileSystem.exists(folder))
			{
				...
				if(found)
				{
					FreeplayState.instance.runtimeShaders.set(name, [frag, vert]);
	...
	function resetTextTag(tag:String) {
		...
		pee.kill();
		if(pee.wasAdded) {
			FreeplayState.instance.remove(pee, true);
		}
		pee.destroy();
		FreeplayState.instance.modchartTexts.remove(tag);
	}
	...
	function cancelTween(tag:String) {
		if(FreeplayState.instance.modchartTweens.exists(tag)) {
			FreeplayState.instance.modchartTweens.get(tag).cancel();
			FreeplayState.instance.modchartTweens.get(tag).destroy();
			FreeplayState.instance.modchartTweens.remove(tag);
		}
	}
	...
	function cancelTimer(tag:String) {
		if(FreeplayState.instance.modchartTimers.exists(tag)) {
			var theTimer:FlxTimer = FreeplayState.instance.modchartTimers.get(tag);
			theTimer.cancel();
			theTimer.destroy();
			FreeplayState.instance.modchartTimers.remove(tag);
		}
	...
	function cameraFromString(cam:String):FlxCamera {
		switch(cam.toLowerCase()) {
			case 'camhud' | 'hud': return FreeplayState.instance.camHUD;
			case 'camother' | 'other': return FreeplayState.instance.camOther;
		}
		return FreeplayState.instance.camGame;
	}
	...
	public function luaTrace(text:String, ignoreCheck:Bool = false, deprecated:Bool = false, color:FlxColor = FlxColor.WHITE) {
		...
		if(ignoreCheck || getBool('luaDebugMode')) {
			...
			FreeplayState.instance.addTextToDebug(text, color);
			trace(text);
		}
		#end
	}
	...
	static function addAnimByIndices(obj:String, name:String, prefix:String, indices:String, framerate:Int = 24, loop:Bool = false)
	{
		...
		if(FreeplayState.instance.getLuaObject(obj, false)!=null) {
			var pussy:FlxSprite = FreeplayState.instance.getLuaObject(obj, false);
	...
	{
		var coverMeInPiss:Dynamic = FreeplayState.instance.getLuaObject(objectName, checkForTextsToo);
	...
	{
		return FreeplayState.instance;
	}
}

class FreeplayModchartSprite extends FlxSprite
{
...
class FreeplayModchartText extends FlxText
{
	public var wasAdded:Bool = false;
	public function new(x:Float, y:Float, text:String, width:Float)
	{
		...
		cameras = [FreeplayState.instance.camHUD];
		scrollFactor.set();
		borderSize = 2;
	}
}

class FreeplayDebugLuaText extends FlxText
{
	private var disableTime:Float = 6;
	public var parentGroup:FlxTypedGroup<FreeplayDebugLuaText>;
	public function new(text:String, parentGroup:FlxTypedGroup<FreeplayDebugLuaText>, color:FlxColor) {
...
class FreeplayCustomSubstate extends MusicBeatSubstate
{
	public static var name:String = 'unnamed';
	public static var instance:FreeplayCustomSubstate;

	override function create()
	{
		instance = this;

		FreeplayState.instance.callOnLuas('onCustomSubstateCreate', [name]);
		super.create();
	...
	override function update(elapsed:Float)
	{
		...
		super.update(elapsed);
		FreeplayState.instance.callOnLuas('onCustomSubstateUpdatePost', [name, elapsed]);
	}

	override function destroy()
	{
		FreeplayState.instance.callOnLuas('onCustomSubstateDestroy', [name]);
		super.destroy();
	}
}

#if hscript
class FreeplayHScript
{
	...
	public function new()
	{
		...
		interp.variables.set('game', FreeplayState.instance);
		interp.variables.set('Paths', Paths);
		interp.variables.set('Conductor', Conductor);
		interp.variables.set('ClientPrefs', ClientPrefs);
		interp.variables.set('Character', Character);
		interp.variables.set('Alphabet', Alphabet);
		interp.variables.set('CustomSubstate', FreeplayCustomSubstate);
		#if (!flash && sys)
		...
		{
			FreeplayState.instance.variables.set(name, value);
		});
		interp.variables.set('getVar', function(name:String)
		{
			var result:Dynamic = null;
			if(FreeplayState.instance.variables.exists(name)) result = FreeplayState.instance.variables.get(name);
			return result;
		});
		interp.variables.set('removeVar', function(name:String)
		{
			if(FreeplayState.instance.variables.exists(name))
			{
				FreeplayState.instance.variables.remove(name);
				return true;
			}
			return false;
		});
	...
	{
		@:privateAccess
		FreeplayHScript.parser.line = 1;
		FreeplayHScript.parser.allowTypes = true;
		return interp.execute(FreeplayHScript.parser.parseString(codeToRun));
	}
}
#end
